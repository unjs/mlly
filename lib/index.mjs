...
// Syntax Utils
const ESM_IMPORT_RE2 = /^\s*import\s*(["'\s]*(?<imports>[\w*${}\n\r\t, /]+)from\s*)?["']\s*(?<from>.*[@\w_-]+)\s*["'][^\n]*$/gm
...
export function matchImports (code) {
  ...
  for (const match of code.matchAll(ESM_IMPORT_RE2)) {
    const { 0: code, index: location, groups: { from, imports = '' } } = match
    importMatches.push({
      code,
      location,
      from,
...
export function parseImport ({ code, imports }) {
  if (code.match(TYPE_ONLY_IMPORT)) {
    return {
      isType: true
    }
  }
