...
// Syntax Utils
const ESM_IMPORT_RE2 = /^\s*import\s*(["'\s]*([\w*${}\n\r\t, /]+)from\s*)?["'\s](.*[@\w_-\s]+)["'][^\n]*$/gm
...
export function matchESMImports (code) {
  ...
  for (const match of code.matchAll(ESM_IMPORT_RE2)) {
    const code = match.shift()
    const from = match.pop().trim()

    const cleanedImports = match.pop()?.replace(COMMENT_RE, '').replace(WHITESPACE_RE, ' ')
    const namedImports = cleanedImports?.match(NAMED_IMPORT_RE)?.[1]?.split(',')
      .map((i) => {
        const [, source = i.trim(), importName = source] = i.match(/^\s*([^\s]*) as ([^\s]*)\s*$/) || []
        return [source, importName]
      })
    const namespacedImport = cleanedImports?.match(/\* as \s*([^\s]*)/)?.[1]
    const defaultImport = cleanedImports?.replace(NAMED_IMPORT_RE, '').split(',').find(i => !i.match(/[*{}]/))?.trim() || undefined
    ...
    imports.push({
      ...
      imports: {
        defaultImport,
        namespacedImport,
        namedImports: Object.fromEntries(namedImports || [])
      }
    })
  }
  return imports
