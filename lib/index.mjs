...
// Syntax Utils
const ESM_IMPORT_RE2 = /(^|;|[^;]*=\s*)(await\s)?\s*import\s*(?<typeIndicator>type|\()?\s*(["'\s]*(?<imports>[\w*${}\n\r\t, /]+)from\s*)?["']\s*(?<from>.*[@\w_-]+)\s*["'][^\n]*$/gm
const TYPE_ONLY_IMPORT = /import type/
const NAMED_IMPORT_RE = /\{([^}]*)\}/
const NAMED_TYPE_IMPORT_RE = /(type \{([^}]*)\}|type [^\s]+)(\s*as\s*[^\s]*)?/g
const DYNAMIC_IMPORTS_RE = /^\s*(const|let|var)\s+(.*)=/
const COMMENT_RE = /(\/\/[^\n]*\n|\/\*.*\*\/)/g
const WHITESPACE_RE = /\s+/g

export function matchESMImports (code) {
  return matchImports(code).map(details => ({
    ...details,
    ...parseImport(details)
  }))
}
...
export function matchImports (code) {
  ...
  for (const match of code.matchAll(ESM_IMPORT_RE2)) {
    const { 0: code, index: location, groups: { from, typeIndicator = 'static', imports = '' } } = match
    const type = typeIndicator === '(' ? 'dynamic' : typeIndicator
    importMatches.push({
      code,
      location,
      from,
...
export function parseImport ({ code, type, imports }) {
  // We won't do further parsing of type imports or dynamic imports (as they require more context)
  if (type === 'type' || code.match(TYPE_ONLY_IMPORT)) {
    return { type: 'type' }
  }

  if (type === 'dynamic') {
    imports = code.match(DYNAMIC_IMPORTS_RE)?.[2] || imports
  }
