...
export async function loadModule (id: string, options: EvaluateOptions = {}): Promise<any> {
  const url = await resolve(id, options);
  const code = await loadURL(url);
  return evalModule(code, { ...options, url });
}

export async function evalModule (code: string, options: EvaluateOptions = {}): Promise<any> {
...
export function transformModule (code: string, options?: EvaluateOptions): Promise<string> {
...
export async function resolveImports (code: string, options?: EvaluateOptions): Promise<string> {
